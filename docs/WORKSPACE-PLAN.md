...
2. **"拖拽文件到窗口打开"受阻**。Wails v2.5.1 **没有** `OnFileDrop`
   （该能力是 2.7+ 才加入的）；而在 WebView2 里，拖放进来的 `File` 对象拿不到真实磁盘路径
   （这是浏览器安全模型，不是 Wails 的问题）。要支持它就得把 Wails 运行时从 2.5.1 升上去。
   为一个便利功能去动核心依赖，风险与收益不成比例，因此先搁置——
   如果需要，建议单独做一次升级并完整回归，而不是夹在别的改动里。
