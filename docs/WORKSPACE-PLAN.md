...
- [x] 大文档性能：**按可见区渐进渲染**
- [ ] 缩略图虚拟滚动（DOM 节点仍然全量，见下）—— 尚未做
- [ ] 缩略图尺寸调节、单页/双页/连续视图切换 —— 尚未做
- [ ] 快捷键面板、右键菜单、拖拽到窗口直接打开 —— 尚未做
- [ ] 深色主题、界面文案统一 —— 尚未做

**验收结果（性能）**：打开一份 300 页的文档，实际只渲染 **7 张缩略图**，
...
**为什么没做虚拟滚动**：300 页时 DOM 里 300 个简单行并不构成瓶颈，
真正的开销是"渲染 300 张图"，而那已经解决了。虚拟滚动会与现有的
拖拽重排（依赖真实元素几何位置）相互干扰，收益不抵风险。等真实使用中
出现上千页卡顿时再上，届时用实测数据驱动。
