...
- [x] 遮罩（拉框 + 颜色/透明度）
- [ ] 水印 / 页码 / 页眉页脚（作用域 = 选中页或全部）—— **尚未开始**
- [ ] 删除批注 / 提取该页文本与图片 —— **尚未开始**

**验收结果（裁剪 / 遮罩）**：坐标换算与落盘均已验证，见下。

关键决策：**裁剪与遮罩做成非破坏式** —— 只写进清单里页面项的 `ops` 字段，
不碰任何文件，导出时才在 `ws-build` 里落到 PDF。理由有三：撤销天然可用；
可以随时改主意；同一份源文件能派生出多个不同结果。
若按上游做法（整份文档跑一次命令产出新文件），这些都做不到。
...
另外补了 **一键重建脚本 `scripts/build.ps1`**：改了 `thirdparty/*.py` 之后必须
重新冻结 `pdf.exe`，而这步极易遗漏——构建看起来成功了，程序里跑的还是旧逻辑。
本阶段就因为漏掉它，出现"导出引擎单测全过、界面导出却没有裁剪"的怪象，
白费一轮排查。脚本把"冻结 → 构建 → 组装运行时文件"固化成一条命令。
