...
3. **没做深色主题**。ant-design-vue 3 只有整份的 `antd.dark.css`（725KB），
   而 Vite 会把所有 import 的 CSS 打进同一个产物，运行时无法在明/暗两套之间切换；
   变量版 `antd.variable.css` 则需要自己补齐整套暗色变量。
   也就是说这需要改造样式加载方式，属于有实质风险的结构性改动，而不是"加个开关"。
   相对其纯装饰性的收益，暂缓。
