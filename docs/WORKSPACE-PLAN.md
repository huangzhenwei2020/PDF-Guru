...
- 拖拽：**手写 HTML5 DnD**（原计划倾向 `vuedraggable@4`，但该库停更于 2021 年，
  而本项目的构建链是 Vite 3 + Node 24 + Vue 3.2；为一次拖拽引入停更依赖不划算）。
  手写之后"拖动选区中任意一项即拖动整组"是天然成立的，插入指示线也只多了十几行 CSS。
...
1. **拖拽改为手写 HTML5 DnD，没有引入 `vuedraggable`。**
   计划里倾向引库，但构建链是 2022 年的（Vite 3 + Node 24 + Vue 3.2），
   而 vuedraggable@4 停更于 2021 年，为一次拖拽引入一个停更依赖不划算。
   手写后多选整组拖动是天然成立的（拖动选区中任意一项即拖动整组），
   插入指示线也只多了十几行 CSS。
2. **旋转用 CSS transform 表现，不在后端重渲染。**
   这样旋转是零延迟的，且与"编辑只改内存模型"的整体设计一致；
   真正的页面旋转角记录在模型里，导出时才落到 PDF。

另外补了一套**无人值守验证手段**（见第 6 节），因为拖拽这类交互没法用截图脚本触发：
